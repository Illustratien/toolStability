...
The default data set `Data` is the subset of APSIM simulated wheat data set, which includes 5 genotypes in 4 locations for 4 years, with 2 nitrogen application rates, 2 sowing dates, and 2 CO2 levels of treatments [@casadebaig2016].
...
| :--- | ---: | :--- |
| Trait |   | Wheat yield (kg.ha<sup>-1</sup>).| 
...
| CO<sub>2</sub> | 2 | CO<sub>2</sub> concentration levels.|
...
3. Function Features
    ...
    User can specify the interested combination of environments by entering a vector with column names of environmental factors. Set `normalize = TRUE` allow user to compare between different stability indices. 
...
Reference equations for stability indices used in the package is listed below:
...
### genotypic superiority measure
Genotypic superiority measure [@lin1988] is calculated based on regression function.
...
Safety-first index [@eskridge1990] is calculated based on the normality assumption of trait over the environments.
 Among different environments, trait below a given critical level \eqn{\lambda} is defined as failure of trait.
