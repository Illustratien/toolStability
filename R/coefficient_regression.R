...
#'
#' @description
#' \code{coefficient_regression} calculate variance of a genotype across environments.
#'
...
#' \deqn{ b_{i}=1 + \frac{sum_{j}(X_{ij} -bar(X_{i.})_{i.})(bar(X_{.j}-bar(X_{..})))}
#' {\sum_{j}(bar(X_{.j}-bar(X_{..})))^{2}}
#'
...
#' @examples
#' data(Data)
#' coef.regression <- coefficient_regression(Data$Yield,Data$Genotype,Data$Environment)
#'
coefficient_regression <- function(trait,genotype,environment){
  ...
  return(res[ ,c("Genotype","coefficient.regression")] )
}
